...
class especialidad
{
    ...
    {
        try {
            $stmt = $this->conex->prepare("SELECT * FROM vs_vista_especialidades");
            $stmt->execute();
    ...
    public function Insertar(especialidad $usr)
    {
        try {
            // Llamada al procedimiento almacenado InsertarRol
            $stmt = $this->conex->prepare("CALL sp_insertar_especialidad(:nombre)");
    ...
    public function Editar(especialidad $usr)
    {
        try {
            // Llamada al procedimiento almacenado EditarRol
            $stmt = $this->conex->prepare("CALL sp_editar_especialidad(:codEspecialidad, :nombre)");
    ...
    public function Eliminar($codEspecialidad)
    {
        try {
            // Llamada al procedimiento almacenado EliminarROL
            $stmt = $this->conex->prepare("CALL sp_eliminar_especialidad(:codEspecialidad)");
            $stmt->bindParam(':codEspecialidad', $codEspecialidad);
            $stmt->execute();
        } catch (PDOException $e) {
            die(json_encode(["success" => false, "message" => $e->getMessage()]));
        }
